...
export default function ResetPasswordPage() {
  ...
  async function submit(event) {
    ...
    try { const data = await apiRequest(API_ENDPOINTS.auth.resetPassword, { method: "POST", body: JSON.stringify({ token, password }) }); setMessage(data.message); toast.success("Password updated successfully."); } catch (requestError) { setError(requestError.message); toast.error(requestError.message); } finally { setLoading(false); }
  }
