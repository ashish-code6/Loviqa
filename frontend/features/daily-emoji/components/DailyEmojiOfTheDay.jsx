...
export default function DailyEmojiOfTheDay() {
  ...
  const emoji = result?.emoji || "\u{1F60A}";
    ...
    <section className="relative overflow-hidden rounded-3xl border border-fuchsia-200/20 bg-gradient-to-br from-fuchsia-400/[0.16] via-violet-500/[0.12] to-cyan-400/[0.08] p-5 shadow-[0_24px_65px_rgba(0,0,0,.25),inset_0_1px_0_rgba(255,255,255,.22)] backdrop-blur-xl sm:p-6">
      ...
      <div className="relative flex items-start justify-between gap-5">
        <div>
          ...
          <p className="mt-1 text-sm text-white/60">The feeling everyone is sharing today.</p>
          <p className="mt-4 inline-flex items-center gap-1.5 text-xs font-semibold text-white/65"><Users className="h-3.5 w-3.5" /> {isLoading ? "Finding today's mood..." : `${count} ${count === 1 ? "person" : "people"} chose this`}</p>
        </div>
        <div className="grid h-20 w-20 shrink-0 place-items-center rounded-3xl border border-white/25 bg-white/[0.13] text-5xl shadow-[inset_0_1px_0_rgba(255,255,255,.32)]">{emoji}</div>
      </div>
    </section>
  );
}
